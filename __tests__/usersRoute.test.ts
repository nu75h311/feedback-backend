...
describe('/api/users', () => {
    ...
    describe('POST', () => {
        ...
        test('POST with non existent schema key', async () => {

            // when POST a user with a non existent field
            const response = await request(app)
                .post('/api/users')
                .send({
                    name: 'Created User',
            ...
            // then response should be OK and created user should be returned without the non existent field
            expect(response.status).toBe(CREATED);
            expect(response.body._id).toBeDefined;
            expect(response.body.name).toBe('Created User');
            expect(response.body.email).toBe('created.user@gmail.com');
